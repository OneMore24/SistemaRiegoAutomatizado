...
// Hi_filtrado(t) = lambda*Hi_sensor(t) + (1-lambda)*Hi_filtrado(t-1)
// ─────────────────────────────────────────────────────────────────────────────
class SensorEMAFilter {
    double lambda_;
    std::vector<double> filtered_;
    std::mt19937 rng_{42};
    std::normal_distribution<double> noise_{0.0, 2.5};
...
public:
    ...
    std::vector<double> read(const std::vector<IrrigationZone>& zones) {
        ...
        for (int i = 0; i < static_cast<int>(zones.size()); ++i) {
            ...
            raw = std::max(0.0, std::min(100.0, raw));
...
// ─────────────────────────────────────────────────────────────────────────────
// Simulador de Clima (perfil diario Ica, Peru)
// ─────────────────────────────────────────────────────────────────────────────
...
public:
    ClimateData next() {
        ...
        double rain  = (rain_chance_(rng_) < 0.05)
                     ? (5.0 + rain_chance_(rng_) * 10.0) : 0.0;
        ++hour_;
